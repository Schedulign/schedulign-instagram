...
export async function lint(posts, { online = true } = {}) {
  ...
  for (const p of posts) {
    for (const f of ['title', 'pillar', 'caption', 'slides']) if (!p[f]) say(p, `missing ${f}`);
    ...
    if (p.pillar === 'spotlight' && !p.claims?.length) say(p, 'a spotlight must name the help article behind its claim');
