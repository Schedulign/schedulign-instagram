/**
 * Publishes the next Schedulign post to Instagram (adapted from the LuxyBasement publisher).
 *
 ...
 * in published.json and not on hold (`hold: true` in its content/posts file).
 * The workflow then commits published.json, so the queue picks up where it
 * left off next time. posts.json is written by render.mjs; never edit it by hand.
 *
...
async function main() {
  ...
  const next = posts.find((p) => !p.hold && (!state[p.id] || (state[p.id].status === 'failed' && state[p.id].attempts < MAX_ATTEMPTS)));
  ...
  if (!next) return console.log('Queue is empty. The weekly job adds the next posts.');
